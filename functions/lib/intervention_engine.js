...
function noneDecision({ accountUid, now, reason, state, decisionId }) {
  return {
...
export function evaluateIntervention({
  ...
  if (profile.setupComplete !== true) {
    return noneDecision({ accountUid, now: current, reason: 'setup_incomplete', state, decisionId });
  }
  if (!activeTasks.length) {
    return noneDecision({ accountUid, now: current, reason: 'no_active_habits', state, decisionId });
  }
  if (!observedCount) {
    return noneDecision({ accountUid, now: current, reason: 'insufficient_history', state, decisionId });
  }
  if (completionRate >= AUTONOMOUS_COMPLETION_THRESHOLD) {
    return noneDecision({ accountUid, now: current, reason: 'autonomous_completion', state, decisionId });
  }
