...
test('D-145: with no config/council document, falls back to the cheapest '
  + 'tier', async () => {
...
test('D-145: a configured model is read from config/council', async () => {
  _resetModelCacheForTest();
...
test('D-145: the result is cached — a second call within the TTL does not '
...
test('D-145: the cache expires after CACHE_TTL_MS and re-reads', async () => {
  _resetModelCacheForTest();
  const store = new FakeFirestore({ 'config/council': { model: 'claude-opus-5' } });
  const now = Date.now();
  await getCouncilModel(store, now);
...
test('D-145: an unreachable Firestore falls back rather than throwing',
  async () => {
    _resetModelCacheForTest();
