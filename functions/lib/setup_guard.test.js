...
test('D-148: no-ops without a uid, sessionId, or store', async () => {
...
test('D-148: the first call counts to 1', async () => {
...
test('D-148: calls accumulate across invocations for the same session', async () => {
  const store = new FakeFirestore();
  for (let i = 0; i < 5; i++) await guardAndCountSetupCall('u1', 's1', store);
  assert.equal(store.data[path('u1', 's1')].modelCallCount, 5);
});

test('D-148: the 40th call is allowed through — it is the one that must '
...
test('D-148: the 41st call is refused', async () => {
...
test('D-148: two different sessions for the same account count '
  + 'independently', async () => {
  const store = new FakeFirestore({ [path('u1', 's1')]: { modelCallCount: SETUP_CALL_LIMIT } });
