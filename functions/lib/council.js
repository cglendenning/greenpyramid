// Pure logic for the Council backend route (D-022/D-145/D-030), split out
...
// (D-148's sibling defense — AiGuard.sanitizeField does the same job
...
// D-145: builds the system prompt (the cacheable stable prefix, D-145) and
...
// D-138: [firstName] — present for category re-clarification (post-setup,
...
// gracefully either way, same pattern as every other D-138 prompt.
export function buildAdvisorTurnPrompt({
  ...
  const systemText =
    ...
    // D-076-adjacent fix, found live 2026-09-10: a new advisor's first-ever
  ...
  // change (D-145: the system block must stay a static, cacheable
  // per-advisor prefix — this varies per call, so it belongs in the
  // user message, the same discipline D-082's convergence nudge and
  // D-079's pyramid context already follow).
...
// D-079: the general Council conversation (D-075) needed real grounding —
...
// D-138: [firstName] — always available here in practice, since this
...
export function buildGeneralCouncilTurnPrompt({
  ...
  const systemText =
    ...
    (name
      ...
      : '') +
    `Otherwise address your fellow advisors, referring to the person in the third person.\n` +
    // D-082: found live — a direct question about the Council itself got
    ...
    // conflicts with D-145's caching (this system block is cacheable only
    ...
    // D-082: found live — a real, evidence-backed connection to the user's
  ...
  // D-145: kept out of the system prompt, same discipline buildAdvisorTurnPrompt
  ...
  // D-082: the push toward a concrete next step lives here, in the
  ...
  // every time the gate flips, defeating D-145's caching for every advisor.
...
// D-074: setup is now a one-on-one conversation with Mira alone, not the
// four-advisor group chat buildAdvisorTurnPrompt above builds — that
// mechanic moved to the general Council conversation (D-075) instead of
...
//
// D-077: [existingCategories] switches this from "gathering material to
...
export function buildSetupAdvisorTurnPrompt({
  ...
  // D-093: the non-refining branch's reply, on the turn Mira decides she's
  ...
  // discipline D-076's pacing reassurance already established.
  // D-094: found live — the model's own readyToBuild judgment on the turn
  ...
  // D-095: found live — once D-076's pacing reassurance ("almost there",
  ...
  // D-076's own intent. This forces exactly one more question at
  // turnsSoFar == 3, the same as D-093/D-094's other "don't trust the
...
// D-076: how many advisor turns are already in the conversation — shared
// by [applyPacingReassurance] below (Mira's solo setup turns) and, despite
// the name, also by index.js's D-082 convergence-nudge gate for the
...
// D-093: the fixed close of the opening conversation, appended
// deterministically by index.js the first time Mira decides she's ready to
// build — never left to the model to phrase consistently, matching D-076's
...
// D-093: true once Mira's fixed wrap-up question already appears in this
...
// D-074: forced tool-use schema for the solo setup turn above — the
