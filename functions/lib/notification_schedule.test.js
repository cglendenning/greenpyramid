...
test('D-149: exactly three daily slots, matching the canonical 9am/2pm/7pm '
  + 'cadence', () => {
...
test('D-149: a UTC user at exactly 09:00 is in the morning window', () => {
  const now = new Date('2026-06-15T09:00:00Z');
  assert.ok(isNotificationWindow('UTC', now));
});

test('D-149: a UTC user at 09:10 is still in the window (within the job '
  + 'interval)', () => {
...
test('D-149: a half-hour-offset timezone (India, UTC+5:30) still matches '
  + 'its local 9am', () => {
  // 09:00 IST == 03:30 UTC.
...
test('D-149: an unset timezone never matches', () => {
  assert.ok(!isNotificationWindow(undefined, new Date()));
});

test('D-149: an invalid timezone string never matches, rather than '
  + 'guessing a default', () => {
  assert.ok(!isNotificationWindow('Not/ATimezone', new Date()));
});

test('D-149: a lapsed account is never eligible, even inside its notification '
  + 'window', () => {
  const now = new Date('2026-06-15T09:00:00Z');
