...
class _SubscriptionStatusState extends State<SubscriptionStatus>
  ...
  Widget _buildStatus(BuildContext context) {
    ...
    if (entitlement != null) {
      ...
      if (kDebugMode) {
        print('[DEBUG] Entitlement originalPurchaseDate: ' +
            entitlement.originalPurchaseDate.toString());
      }
    }
    ...
    DateTime? planPurchaseDate = planPurchaseDateStr != null
        ? DateTime.tryParse(planPurchaseDateStr)
        : null;
    ...
    if (planPurchaseDate == null && entitlement != null) {
      try {
        ...
        if (kDebugMode) {
          print(
              '[DEBUG] - Using entitlement latestPurchaseDate as fallback: $planPurchaseDate');
        }
      } catch (e) {
    ...
    if (planPurchaseDate == null && parsedExpiration != null) {
      try {
        ...
        final estimatedPurchaseDate =
            parsedExpiration.subtract(const Duration(days: 30));
        if (estimatedPurchaseDate.isBefore(DateTime.now())) {
          planPurchaseDate = estimatedPurchaseDate;
          if (kDebugMode) {
            print(
                '[DEBUG] - Using estimated purchase date (expiration - 30 days): $planPurchaseDate');
          }
        }
      } catch (e) {
    ...
    String purchased = planPurchaseDate != null
        ? DateFormat.yMMMMd().add_jm().format(planPurchaseDate.toLocal())
        : 'Unknown';
    ...
    if (kDebugMode) {
      ...
      print(
          '[DEBUG] - All entitlements (including inactive): ${_customerInfo?.entitlements.all.keys}');
      print(
          '[DEBUG] - Latest expiration date: ${_customerInfo?.latestExpirationDate}');
      print('[DEBUG] - All purchase dates: ${_customerInfo?.allPurchaseDates}');
      print(
          '[DEBUG] - Non subscription transactions: ${_customerInfo?.nonSubscriptionTransactions}');
      print('[DEBUG] - Plan ID for purchase date lookup: $planId');
      print(
          '[DEBUG] - Purchase date from allPurchaseDates: $planPurchaseDateStr');
    ...
    for (final entry in allEntitlementsMap.entries) {
      ...
      print(
          '[DEBUG] -   - latestPurchaseDate: ${entitlement.latestPurchaseDate}');
      print(
          '[DEBUG] -   - originalPurchaseDate: ${entitlement.originalPurchaseDate}');
      ...
      try {
        ...
        if (entitlement.willRenew == false &&
          ...
          if (expDate != null && expDate.isAfter(DateTime.now())) {
            print(
                '[DEBUG] -   - FOUND CANCELLED BUT NOT EXPIRED! (willRenew = false)');
        ...
        if (activeSubs.isNotEmpty &&
          ...
          if (expDate != null && expDate.isAfter(DateTime.now())) {
            print(
                '[DEBUG] -   - FOUND CANCELLED BUT NOT EXPIRED! (active subs but inactive entitlement)');
            isCancelledButNotExpired = true;
            break;
          }
        }
        ...
        if (!entitlement.isActive && entitlement.expirationDate != null) {
          ...
          if (expDate != null && expDate.isAfter(DateTime.now())) {
            print(
                '[DEBUG] -   - FOUND CANCELLED BUT NOT EXPIRED! (inactive entitlement)');
            isCancelledButNotExpired = true;
            break;
          }
        }
    ...
    if (!isCancelledButNotExpired) {
      isCancelledButNotExpired = !isActive &&
          parsedExpiration != null &&
          parsedExpiration.isAfter(DateTime.now());
    }

    print(
        '[DEBUG] - Final isCancelledButNotExpired: $isCancelledButNotExpired');
