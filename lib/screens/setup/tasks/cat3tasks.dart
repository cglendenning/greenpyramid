...
import 'package:life_ops/screens/setup/setup14.dart';
...
List<String> cat3TaskChoices = <String>[defaultText];
List<SetupTask> tasksForCat3 = [];
bool cat3TasksGenerating = false;

class Cat3Tasks extends StatefulWidget {
  final String cat;

  const Cat3Tasks(this.cat);

  @override
  _Cat3TasksState createState() => _Cat3TasksState(cat);
}

class _Cat3TasksState extends State<Cat3Tasks> {
  String taskValue = cat3TaskChoices.first;
  String cat;
  ...
  Widget build(BuildContext context) {
    analytics.logEvent(name: '${setupVersion}_cat3tasks');
    ...
    return SafeArea(
        child: Scaffold(
            ...
            body: Center(
              ...
              ProgressBar(currentStep: 16, totalSteps: 23),
  ...
  showSkipAlertDialog(BuildContext context) {
    ...
    Widget continueButton = TextButton(
      ...
      onPressed: () {
        setState(() {
          analytics.logEvent(name: 'skip_cat3tasks');
          currentCatId = 1;
          currentTaskId = 0;
          tasksForCat3.clear();
          cats.clear();
  ...
  Future<String> generateCat3TaskList() async {
    ...
    print(chatResult);
    cat3TaskChoices.clear();
    cat3TaskChoices = chatResult.split("|");
    cat3TaskChoices.add("Enter My Own...");
    taskValue = cat3TaskChoices.first;

    setState(() {});

    cat3TasksGenerating = false;
    return chatResult;
  }
  ...
  void navigateToSetup14() async {
    ...
    await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => const Setup14()),
    ).then((value) => currentCatId = 3);
  }
