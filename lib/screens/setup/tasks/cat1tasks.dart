...
import 'package:life_ops/screens/setup/tasks/cat2tasks.dart';
...
List<String> cat1TaskChoices = <String>[defaultText];
List<SetupTask> tasksForCat1 = [];
bool cat1TasksGenerating = false;

class Cat1Tasks extends StatefulWidget {
  final String cat;

  const Cat1Tasks(this.cat);

  @override
  _Cat1TasksState createState() => _Cat1TasksState(cat);
}

class _Cat1TasksState extends State<Cat1Tasks> {
  String taskValue = cat1TaskChoices.first;
  String cat;
  ...
  Widget build(BuildContext context) {
    analytics.logEvent(name: '${setupVersion}_cat1tasks');
    ...
    return SafeArea(
        child: Scaffold(
            ...
            body: Center(
              ...
              ProgressBar(currentStep: 14, totalSteps: 23),
  ...
  showSkipAlertDialog(BuildContext context) {
    ...
    Widget continueButton = TextButton(
      ...
      onPressed: () {
        setState(() {
          analytics.logEvent(name: 'skip_cat1tasks');
          currentCatId = 1;
          currentTaskId = 0;
          tasksForCat1.clear();
          cats.clear();
  ...
  void navigateToCat2Tasks() async {
    ...
    await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => Cat2Tasks(foundCat.cat)),
    ).then((value) => currentCatId = 1);
  }
