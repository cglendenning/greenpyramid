...
class AuthService {
  ...
  /// `WelcomeScreen`; the earlier design eagerly re-created an anonymous
  /// session immediately after every sign-out and then needed a
  /// persisted flag to remember that a sign-out had just happened, since
  /// the fresh anonymous session made that fact otherwise unrecoverable.
  /// Removed entirely, per the owner's explicit correction: state that
  /// can be read live from Firebase itself should never be shadowed by
  /// app-managed, persisted history. Callers that need an anonymous
  /// session again (linking a credential, entering setup) call
  /// [signInSilently] themselves, lazily, exactly when they need it —
  /// see `AccountLinkService.signInWithApple`/`signInWithGoogle`.
