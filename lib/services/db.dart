...
class DatabaseHelper {
  ...
  // into the (now-deleted) wizard. 9: D-098 — a habit's optional recurring
  // scheduled time and its native calendar event id. 10: D-099 — a
  // tasklog row's optional voice-transcribed miss reason. 11: D-098 — a
  // habit's own scheduled-event duration, in minutes. 12: D-122 — the
  // newsfeed_item table (personal, on-device newsfeed). 13: D-122 — a
  // one-time wipe of newsfeed_item, discarding rows generated by earlier
  // iterations of D-122/D-122/D-122's copy and item types before they
  ...
  // the newsfeed. 14: D-122 — a second, narrower one-time wipe: only
  ...
  // rather than only tomorrow. 15: D-122 — a third, narrower one-time
  // wipe: only type='welcome' rows, discarding the two welcome cards
  // seeded (by D-122's own wipe, on the previous build) with the old,
  // adjacent-at-the-top timestamps, so they reseed with D-122's spacing
  // fix in effect the next time anything touches the newsfeed. 16:
  // D-122 — a fourth, narrower one-time wipe: only type='essence' rows,
  // discarding every essence card generated under the old flat
  // "{cat}, redefined." template so they all regenerate under the new
  // first-definition/redefinition-aware framing. 17: D-122 — collapses
  ...
  // regenerate from the corrected history. 18: D-122 — a database-level
  ...
  // insertCategoryEssence's own D-122 guard. 19: D-122 — retires the two
  ...
  // had them. 20: D-122 (backfill) — an unconditional full wipe of
  // newsfeed_item, so every card on an existing account regenerates
  // fresh under everything the newsfeed accumulated across D-122
  // through D-122, not just whichever single round each prior narrow
  // migration targeted.
  ...
  // D-098: a habit's optional recurring scheduled time. Null (the
  ...
  // D-098: how long the habit's scheduled block/native-calendar event
  ...
  // D-099: the voice-transcribed reason a scheduled habit's batch
  ...
  // the category name, so a rename cannot orphan it (D-059, II-N).
  ...
  // D-122: the personal newsfeed — items generated entirely on-device from
  ...
  /// Set when the database cannot be opened or migrated (D-147). The app shows
  ...
  _initDatabase() async {
    ...
    try {
      ...
      // D-147: migration is best-effort. A database we cannot open is not
  ...
  static Future<void> applyV7Schema(Database db) async {
    ...
    // D-122: a fresh install gets the duplicate-guard trigger from day
  ...
  /// D-122: the newsfeed_item table, written idempotently (MIG-4) so it's
  ...
  /// D-122: the v16->v17 migration's own cleanup — deletes every essence
  ...
  /// D-122: a database-level backstop for the same invariant D-122's
  ...
  Future _onCreate(Database db, int version) async {
    ...
    // D-122: the newsfeed table, shared with the v12 migration.
      ...
      if (i > oldVersion) {
        switch (i) {
          ...
          case 8:
            ...
            break;
          case 9:
            // D-098: a habit's optional recurring scheduled time, and the
            ...
            break;
          case 10:
            // D-099: a tasklog row's optional voice-transcribed miss
            ...
            break;
          case 11:
            // D-098: a habit's own scheduled-event duration — null on
          ...
          case 13:
            ...
            break;
          case 14:
            // D-122: found live — the very first real article a
            ...
            // type='article' rows (not the whole table, unlike D-122 —
            // the essence/streak/welcome cards D-122 just regenerated
            // are fine and untouched) frees the slot immediately.
            await db.delete(newsfeedItemTable,
                where: '$columnNewsfeedType = ?', whereArgs: ['article']);
            break;
          case 15:
            // D-122: found live — the two welcome cards, seeded
            ...
            break;
          case 16:
            // D-122: found live — every essence-redefinition card, no
            ...
            break;
          case 17:
            // D-122: found live, on the owner's own account — a category
            ...
            // reinstalls each triggered a restore via D-109). Fixed at
          ...
          case 19:
            ...
            // place, the same self-limiting trigger pattern D-122
            // established.
            await db.delete(newsfeedItemTable,
                where: '$columnNewsfeedType = ?', whereArgs: ['welcome']);
            break;
          case 20:
            // D-122 (backfill): owner — "I also need you to clean up my
            // database because I still have cards in there that were
            // generated before this change." D-122/D-122/D-122/D-122's
          ...
          case 23:
            ...
            break;
          case 22:
            // D-138: single account-level personal-info fields, collected
...
//                                                            populateCategory()
// -----------------------------------------------------------------------------
  // D-105: was `void populateCategory() async` — nothing could await it, so
...
// Helper methods
  ...
  // and the value is the column value. The return value is the id of the
  // inserted row.
  // D-038/D-059: categoryid is the PRIMARY KEY, and every fresh install
  ...
  /// D-143: two distinct "nothing to show" cases, two distinct sentinels
  ...
  /// D-147: every task_log row, full stop — the sync source for
  /// `recentActivity`, so a full uninstall/reinstall can restore every
  /// check-off ever recorded, not just a recent window (D-147's original
  /// 250-row bound is gone; see D-147).
  ...
  ///
  /// D-059 will extend this to cascade the rename into `task` and `task_log`,
  ...
  ///
  /// D-059. `task` and `tasklog` key on the category *name*, so renaming
  ...
  /// D-099: records one habit's answer from the batch check-in screen —
  ...
  /// D-138: collected once during setup (a plain text-entry screen right
  ...
  /// D-138: pure personal-reference fields from the Profile screen — no
  ...
  /// D-138: local file path only — the photo itself stays on-device, not
  ...
  /// screen is addressed by name, but essence lookup keys on id (D-059).
  ...
  /// D-145/D-047: the active (most recent) essence for one category, the
  ...
  /// D-079: id, name, tier, and latest essence for every category — the
  ...
  /// D-122: the longest run of consecutive days, ending on the most recent
  ...
  /// D-122: writes one newsfeed item, silently skipped (INSERT OR IGNORE)
  ...
  /// D-122: now returns whether the row was actually inserted (`true`) or
  ...
  /// notification for.
  /// D-122: [createdAt] lets a caller place an item at a deliberate
  ...
  /// D-122: one page of the newsfeed, newest first.
  ...
  /// D-122: a cheap existence check by dedupeKey, used before doing any
  ...
  /// D-122: the 0-based rank of the item identified by [dedupeKey] within
  ...
  /// D-145/D-047: appends a new essence version for a category (essences are
  /// versioned, never overwritten — D-047). D-122: a no-op — returns
