...
class DatabaseHelper {
  ...
  // newsfeed_item table (personal, on-device newsfeed). 13: D-150 — a
  // one-time wipe of newsfeed_item, discarding rows generated by earlier
  // iterations of D-150/D-150/D-150's copy and item types before they
  ...
  // the newsfeed. 14: D-150 — a second, narrower one-time wipe: only
  ...
  // rather than only tomorrow. 15: D-150 — a third, narrower one-time
  // wipe: only type='welcome' rows, discarding the two welcome cards
  // seeded (by D-150's own wipe, on the previous build) with the old,
  // adjacent-at-the-top timestamps, so they reseed with D-150's spacing
  // fix in effect the next time anything touches the newsfeed. 16:
  // D-150 — a fourth, narrower one-time wipe: only type='essence' rows,
  // discarding every essence card generated under the old flat
  // "{cat}, redefined." template so they all regenerate under the new
  // first-definition/redefinition-aware framing. 17: D-150 — collapses
  ...
  // regenerate from the corrected history. 18: D-150 — a database-level
  ...
  // insertCategoryEssence's own D-150 guard. 19: D-150 — retires the two
  ...
  // had them. 20: D-150 (backfill) — an unconditional full wipe of
  // newsfeed_item, so every card on an existing account regenerates
  // fresh under everything the newsfeed accumulated across D-150
  // through D-150, not just whichever single round each prior narrow
  // migration targeted.
  ...
  /// Set when the database cannot be opened or migrated (D-187). The app shows
  ...
  _initDatabase() async {
    ...
    try {
      ...
      // D-187: migration is best-effort. A database we cannot open is not
  ...
  static Future<void> applyV7Schema(Database db) async {
    ...
    // D-150: a fresh install gets the duplicate-guard trigger from day
  ...
  /// D-150: the v16->v17 migration's own cleanup — deletes every essence
  ...
  /// D-150: a database-level backstop for the same invariant D-150's
      ...
      if (i > oldVersion) {
        switch (i) {
          ...
          case 12:
            ...
            break;
          case 13:
            // D-150: a one-time wipe of every existing newsfeed_item row
            // — found live, an existing install (specifically the one
            // this was developed and tested on) had accumulated rows
            // from D-150/D-150/D-150's copy and item-type iterations
            ...
            break;
          case 14:
            // D-150: found live — the very first real article a
            ...
            // type='article' rows (not the whole table, unlike D-150 —
            // the essence/streak/welcome cards D-150 just regenerated
            // are fine and untouched) frees the slot immediately.
            await db.delete(newsfeedItemTable,
                where: '$columnNewsfeedType = ?', whereArgs: ['article']);
            break;
          case 15:
            // D-150: found live — the two welcome cards, seeded
            ...
            break;
          case 16:
            // D-150: found live — every essence-redefinition card, no
            ...
            break;
          case 17:
            // D-150: found live, on the owner's own account — a category
          ...
          case 19:
            ...
            // place, the same self-limiting trigger pattern D-150
            // established.
            await db.delete(newsfeedItemTable,
                where: '$columnNewsfeedType = ?', whereArgs: ['welcome']);
            break;
          case 20:
            // D-150 (backfill): owner — "I also need you to clean up my
            // database because I still have cards in there that were
            // generated before this change." D-150/D-150/D-150/D-150's
            ...
            // (D-150 through D-150) since the account's own cards were
            // first generated. An unconditional full wipe — the same
            // "regenerate everything fresh" approach D-150 already
...
// Helper methods
  ...
  /// D-187: every task_log row, full stop — the sync source for
  /// `recentActivity`, so a full uninstall/reinstall can restore every
  /// check-off ever recorded, not just a recent window (D-187's original
  /// 250-row bound is gone; see D-187).
  ...
  /// D-187: every version of every category's essence — part of the synced
  /// set.
  ...
  /// D-150: now returns whether the row was actually inserted (`true`) or
  ...
  /// notification for.
  /// D-150: [createdAt] lets a caller place an item at a deliberate
  ...
  /// D-150: a cheap existence check by dedupeKey, used before doing any
  ...
  /// D-150: how many on-demand articles have already been generated
  ...
  /// D-187: accumulated four-domain findings — part of the synced set.
  Future<List<Map<String, dynamic>>> queryAllDomainFindings() async {
    final db = await database;
    return db.query(domainFindingTable);
  }

  /// D-185: findings grouped by domain, for the domain-map view. Domain
  /// state is derived from accumulated findings, never asked of the user
  /// directly (D-185's own acceptance criterion).
