...
class RadarChart extends StatefulWidget {
  ...
  const RadarChart({
    ...
    this.featuresTextStyle = const TextStyle(color: Colors.black, fontSize: 16, fontFamily: 'Exo2', fontWeight: FontWeight.bold),
  ...
  factory RadarChart.dark({
    ...
    return RadarChart(
        ...
        features: features,
        data: data,
        featuresTextStyle: const TextStyle(color: Colors.white, fontSize: 16, fontFamily: 'Exo2', fontWeight: FontWeight.bold),
      ...
      data.asMap().forEach((index, graph) {
        ...
        graph.asMap().forEach((index, point) {
          ...
          if (reverseAxis) {
            ...
            // collect the points for building the arc to close the graph.
            if (point > 0) {
              pointRadius = point.toInt();
              if (point1x == 0) {
                point1x = centerX + scaledPoint * xAngle;
                // point1y = centerY + scaledPoint * yAngle;
              } else {
                // point2x = centerX + scaledPoint * xAngle;
                // point2y = centerY + scaledPoint * yAngle;
              }
            }
            path.lineTo(
        ...
        // This is completely insane. It worked on multiple screen sizes
        // when I wrote it, but may not work forever. If something looks
        // wonky, I need to calculate the value correctly based on the
        // actual radar chart.
        if (point1x > 0) {
          double startAngle = 0;
          switch (index) {
