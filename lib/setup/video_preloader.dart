...
class VideoPreloader {
  ...
  Future<void> startPreloading() async {
    ...
    if (Platform.isAndroid) {
      if (kDebugMode) {
        print(
            '🌐 [VIDEO PRELOADER] Android detected - WebView will handle video loading');
      }
      return;
    }
  ...
  YoutubePlayerController? getPreloadedController() {
    ...
    if (Platform.isAndroid) {
      if (kDebugMode) {
        print(
            '🌐 [VIDEO PRELOADER] Android detected - using WebView instead of preloaded controller');
      }
      return null;
    }
    ...
    if (!_isPreloaded || _preloadedController == null) {
      print(
          '⚠️ [VIDEO PRELOADER] No preloaded controller available, creating new one');
      return null;
    }
    ...
    print(
        '🎬 [VIDEO PRELOADER] Returning preloaded controller for immediate playback');
    return controller;
  }
