...
/// newsfeed_item existed. D-122's own migration (case 17) queries
/// category_essence, and D-139's migration (case 22) alters
...
/// generated by earlier iterations of D-122/D-122/D-122's copy and item
...
/// during development." newsfeed_item is purely local SQLite (D-122:
...
void main() {
  ...
  test(
      'D-122: a newsfeed_item row from an existing v12 database is gone '
  ...
  test(
      'D-122: a fresh install (no existing database at all) is '
  ...
  test(
      'D-122: upgrading from v13 applies every migration in the chain '
      ...
      'by the end of it, D-122\'s own v19->v20 full wipe leaves nothing '
    ...
    expect(rows, isEmpty,
        reason: "the chain's final step (D-122's v19->v20) is an "
            'unconditional full wipe, so nothing of any type survives '
            'reaching the current version from this starting point');
  });

  test(
      'D-122: upgrading from v14 applies every migration in the chain '
      'without error — this directive originally fixed the two welcome '
      "cards' spacing; by the current version, D-122's own full wipe "
  ...
  test(
      'D-122: upgrading from v15 applies every migration in the chain '
      'without error — this directive originally rewrote essence-card '
      "copy; by the current version, D-122's own full wipe (v19->v20) "
  ...
  test(
      'D-122: the v16->v17 migration collapses consecutive same-text '
  ...
  test(
      'D-122: the v17->v18 migration installs the duplicate-guard '
    ...
    // this database was created before D-122 shipped.
  ...
  test(
      'D-122: upgrading from v18 applies every migration in the chain '
      'without error — this directive originally retired the two '
      "generic welcome cards on their own (v18->v19); by the current "
      "version, D-122's own later full wipe (v19->v20) leaves nothing "
      'of any type, which is expected and covered by its own dedicated '
  ...
  test(
      'D-122 (backfill): the v19->v20 migration unconditionally wipes '
      ...
      'under everything the newsfeed accumulated across D-122 through '
      "D-122, not just whichever single round each prior narrow "
  ...
  test(
      'D-132: the v20->v21 migration deletes only streak/essence rows, '
