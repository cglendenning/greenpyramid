...
/// newsfeed_item existed. D-150's own migration (case 17) queries
...
/// D-150: found live on the owner's own device — newsfeed_item rows
/// generated by earlier iterations of D-150/D-150/D-150's copy and item
...
void main() {
  ...
  test(
      'D-150: a newsfeed_item row from an existing v12 database is gone '
  ...
  test(
      'D-150: a fresh install (no existing database at all) is '
  ...
  test(
      'D-150: upgrading from v13 applies every migration in the chain '
      ...
      'by the end of it, D-150\'s own v19->v20 full wipe leaves nothing '
    ...
    expect(rows, isEmpty,
        reason: "the chain's final step (D-150's v19->v20) is an "
            'unconditional full wipe, so nothing of any type survives '
            'reaching the current version from this starting point');
  });

  test(
      'D-150: upgrading from v14 applies every migration in the chain '
      'without error — this directive originally fixed the two welcome '
      "cards' spacing; by the current version, D-150's own full wipe "
  ...
  test(
      'D-150: upgrading from v15 applies every migration in the chain '
      'without error — this directive originally rewrote essence-card '
      "copy; by the current version, D-150's own full wipe (v19->v20) "
  ...
  test(
      'D-150: the v16->v17 migration collapses consecutive same-text '
  ...
  test(
      'D-150: the v17->v18 migration installs the duplicate-guard '
    ...
    // this database was created before D-150 shipped.
  ...
  test(
      'D-150: upgrading from v18 applies every migration in the chain '
      'without error — this directive originally retired the two '
      "generic welcome cards on their own (v18->v19); by the current "
      "version, D-150's own later full wipe (v19->v20) leaves nothing "
      'of any type, which is expected and covered by its own dedicated '
  ...
  test(
      'D-150 (backfill): the v19->v20 migration unconditionally wipes '
      ...
      'under everything the newsfeed accumulated across D-150 through '
      "D-150, not just whichever single round each prior narrow "
